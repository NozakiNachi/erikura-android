...
object ErikuraConfig {
    ...
    const val IDENTIFICATION_REQUIRED = "identification_required"
    ...
    var identificationRequired: Boolean = false
    ...
    fun load(context: Context, onError: ((messages: List<String>?) -> Unit)? = null) {
        if (!loaded) {
            Api(context).erikuraConfig(onError = onError) { result ->
                ...
                result[IDENTIFICATION_REQUIRED]?.let { v ->
                        (v as ErikuraConfigValue.BooleanValue)?.let {
                            identificationRequired =
                                it.value ?: identificationRequired }
                }
                loaded = true
            }
        }
