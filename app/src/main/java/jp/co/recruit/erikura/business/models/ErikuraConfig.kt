...
object ErikuraConfig {
    ...
    fun jobNewReportURLString(job_id: Int?, token: String?): String {
        return BuildConfig.SERVER_BASE_URL + "login_with_token?token="+ token + "&job_id=" + job_id
    }

    fun jobEditReportURLString(report_id: Int?, token: String?): String {
        return BuildConfig.SERVER_BASE_URL + "login_with_token?token="+ token + "&report_id=" + report_id
    }
