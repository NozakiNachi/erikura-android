...
interface IErikuraApiService {
    ...
    @GET("places/cautions")
    fun placeCautions(@Query("job_id") jobId: Int? = null,
                      @Query("place_id") placeId: Int? = null,
                      @Query("job_kind_id") jobKindId: Int? = null): ApiObservable<CautionResponse>
    ...
    override fun deserialize(
        ...
        jsonObject?.entrySet()?.forEach { entry ->
            val deserializerMap: Map<String, (JsonElement?) -> ErikuraConfigValue> = mapOf(
                ...
                Pair(ErikuraConfig.IDENTIFICATION_REQUIRED, { json -> deserializeString(json, context) })
            )
